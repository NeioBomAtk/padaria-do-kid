    ...
    cartItemElement.innerHTML = `
      <div class="flex items-center justify-between">
        <div>
          ...
          ${item.quantity > 1 ? `<p class="font-medium mt-1 text-sm text-gray-500">Subtotal: R$ ${(item.price * item.quantity).toFixed(2).replace('.', ',')}</p>` : ''}
        </div>
...
checkoutBtn.addEventListener("click", function(){
  ...
  const cartItems = cart.map((item) => {
    return (
      ` ${item.name} Quantidade: (${item.quantity}) Preço: R$${item.price.toFixed(2).replace('.', ',')} |`
    )
  }).join("")
  ...
  const formatTitleCase = (text) => {
    return text.split(' ').map(word => {
      ...
      if (['da', 'de', 'do', 'das', 'dos'].includes(lower)) return lower;
      return lower.charAt(0).toUpperCase() + lower.slice(1);
    }).join(' ');
  };

  const address = addressInput.value.replace(/(\d)\.(?=\d)/g, '$1');
